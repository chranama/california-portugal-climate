...
- `proof/workflow_state.latest.json`: latest generated local verification state.
...
- local verification command outcomes
- warehouse row counts for core dbt and observability tables
- latest model metric summary, including minority-class F1 and recall
- prediction CSV row count
- latest pipeline and ML observability summaries
- repo commit and dirty-worktree status at generation time

This artifact is intended to show the latest local workflow state without
requiring separate inference from individual files.
