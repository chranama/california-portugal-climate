...
Current saved metric values should be read from the metrics JSON or from
`proof/workflow_state.latest.json` after `uv run climate-verify-local`.

The high accuracy and ROC-AUC should be read together with the class
distribution and minority-class recall/F1. The baseline model is a workflow
artifact; it does not currently establish strong positive-event detection or a
climate science performance claim.
...
`proof/workflow_state.latest.json` is generated by `uv run climate-verify-local`
and records command outcomes, warehouse row counts, model metrics, prediction
row counts, and observability summaries for the latest local verification workflow.
